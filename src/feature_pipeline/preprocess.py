...
def preprocess_split(
    ...
    Args:
        split: Split name (train, eval, holdout)
        raw_dir: Directory containing raw data
        processed_dir: Directory to save processed data
        metros_path: Path to metros file (optional)
    ...
    Raises:
        ...
        ValueError: If split data is empty
    """
    ...
    try:
        df = pd.read_csv(path)
        logger.info(f"✅ Loaded raw {split} data: {df.shape}")
    except Exception as e:
        logger.error(f"❌ Error loading {split} data: {e}")
        raise
    
    if df.empty:
        raise ValueError(f"Raw {split} data is empty")
    ...
    try:
        ...
        logger.error(f"❌ Error saving preprocessed {split}: {e}")
        raise
