...
def test_target_encode_applies_mapping():
    ...
    assert "city_full_te" in train.columns
    assert eval["city_full_te"].notnull().all()
...
def test_full_pipeline_integration(tmp_path):
    ...
    assert {"year", "zipcode_freq", "city_full_te"}.issubset(out_train.columns)
    assert {"year", "zipcode_freq", "city_full_te"}.issubset(out_eval.columns)
